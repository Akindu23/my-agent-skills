...
**Interactive hub**:
TTY launcher when `my-agent-skills` runs with no subcommand: a Clack `select` menu (↑↓ and Enter; hint shown under the prompt; default highlight always **Add Skill(s)**). **Menu-loop is the default:** the hub runs the chosen action, then re-displays the menu, indefinitely — bare TTY is a return-to-menu loop, not one-shot. After **every** action (read-only or mutating) the hub **pauses** ("press Enter to return to menu") so the result is readable before the menu re-prints; the menu re-prints **below** prior output (scrollback preserved) and the banner shows **once** per process. **Leaving the hub:** the **Quit** item and **Esc-at-menu** share one clean-exit path (print outro, exit 0, or non-zero when any action failed this session — **hadFailure**); **Ctrl+C** hard-quits with **exit 130** from anywhere. **Esc inside an action** abandons it and returns to the menu (all the way back, not one step), with no confirmation — no action writes to disk before its own final confirm, so mid-configuration abandonment leaves nothing half-written (a Ctrl+C *during* materialization can, recovered via **sync**/**check**). On command failure, the menu is shown again for recovery. **Check** from the hub may offer "Update drifted skills now?" when drift is found, then **returns to the menu** like any other action (no special exit). Scripts use explicit subcommands or flag-only `add`. Interactive **`add`** picks **install scope** then **install target** (or, on an existing Cursor-only lock, may offer to add Claude Code and materialize immediately). Interactive **`sync`** / **`update`** do not re-ask target — they use recorded/effective lock **`targets`**.
_Avoid_: default `add` command (removed; bare non-TTY exits with a hint instead); one-shot bare TTY (retired — menu-loop is now the only bare-TTY mode); treating Esc and Ctrl+C as identical (Esc = back/clean-quit, Ctrl+C = hard-quit 130); Esc-at-menu exiting 0 when an action failed; `check` exiting the hub; prompting "for both?" on sync/update.
_Avoid_: `--menu` / `--once` as meaningful flags (**`--menu`** is a deprecated no-op kept only for backward compatibility; there is no `--once` — single-shot bare TTY was retired when menu-loop became the default).
...
**Update command**:
Fetches the remote pack at the latest default-branch commit (or reapplies the current pin when only **lock–source hash mismatch** exists), then refreshes skills already listed in **cursor-skills-lock.json** for a scope. Does not install skills the user never selected—use **add** for that—but **does** complete **missing dependencies** of skills already in the lock (see below). When **orphan lock entries** or **missing dependencies** exist, interactive runs show a pre-selected **multiselect** for each before the main proceed prompt; **`update -y`** accepts all of both automatically. Summary separates **content changed** from **will relink N skills** on pin advance, plus a **new deps** count when dependencies are pending. Re-materializes using each lock entry’s **linkType** (symlink or copy); lock root **`defaultLinkType`** applies to new **`add`** installs and to accepted missing dependencies.
_Avoid_: `sync` as a substitute for upstream refresh, `npx skills update` (upstream Vercel CLI); treating a missing dependency as equivalent to `add`ing an unrelated skill—it only ever completes something already selected.

**Check command**:
Read-only drift report for a scope: **pack commit drift**, per-skill **changed / unchanged / pin drift / orphan / missing dependency**, and manifest version drift, plus **per-target** materialization health for each recorded/effective **install target**. Exits non-zero when pack drift exists or **any** target is unhealthy; no lock or install writes.
...
**Missing dependency** (drift status `missingDependency`):
A skill named in the current manifest's **dependsOn map** for an already-locked skill, but not itself present in **cursor-skills-lock.json**—the mirror case of **orphan**: instead of a lock entry the manifest no longer wants, it's a manifest requirement the lock doesn't yet satisfy (e.g. `grilling` starts depending on `tdd` after the pack updates). Detected by re-running dependency expansion over the currently-locked skill names against the resolved manifest. **Check** reports it as `missing`; **update** reports it as `new` and offers a pre-selected **multiselect** ("New dependencies required by your installed skills") before the main proceed prompt, mirroring orphan removal in the opposite direction. Non-interactive **`update -y`** installs all of them; without `-y`, non-interactive runs warn and skip. Declining one leaves the owning skill exactly as incomplete as it already was—no new breakage, just a deferred fix. Does not track which lock entries are dependency-only vs. user-selected roots, so it only ever *grows* the lock; pruning a dependency that becomes unused is explicitly out of scope (would need that root/dependency distinction persisted first).
_Avoid_: confusing this with **orphan** (opposite direction: manifest wants it, lock lacks it); auto-removing a dependency that's no longer referenced (not implemented—no root-tracking to make it safe); silently installing it without the multiselect/`-y` gate (would blur `update`'s "never installs anything without consent" guarantee from ADR-0003).
