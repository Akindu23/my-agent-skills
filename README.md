...
| [`council`](./skills/council/) | `council` | Explore a codebase area, spawn **Task** subagents for parallel deep dives, then synthesize results (e.g. multi-area review, reconnaissance before planning). | Self-composed skill; explicit **`composer-2.5`** on every Task (standard only; no fast variants); see [`references/cursor-task-workflow.md`](./skills/council/references/cursor-task-workflow.md). |
