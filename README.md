...
| [`review`](./skills/review/) | `review` | Two-axis diff review (Standards vs Spec) since a user-chosen baseline; parallel **`Task`** runs with `generalPurpose` and explicit **`model`**. | Use **`/review`**; `disable-model-invocation`. Optional **`/setup-matt-pocock-skills`** for issue-linked specs (`docs/agents/issue-tracker.md`). [Matt Pocock — Skills](https://github.com/mattpocock/skills). |
