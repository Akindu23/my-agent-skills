...
Probe Task `model` / `subagent_type` before fan-out. Route per [`../council/references/cursor-task-workflow.md`](../council/references/cursor-task-workflow.md), with these overrides:
...
4. **Specialists.** In **one** message, two parallel Grok-lane Tasks (each gets the package + council brief):
   ...
   - **YAGNI** — `generalPurpose` (readonly). Run the `/yagni` skill (read [`../yagni/SKILL.md`](../yagni/SKILL.md) if not already loaded); review the change set; return findings only.
