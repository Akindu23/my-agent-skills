...
2. Spawn **n={as much as needed by the main agent}** (unless specified otherwise) subagents with the **Task** tool. Before fan-out, **probe** the Task `model` enum and set **`model`** to the first matching Composer slug per [`references/cursor-task-workflow.md`](references/cursor-task-workflow.md). Omit `model` only when the enum lists no Composer slugs (inherit parent).
