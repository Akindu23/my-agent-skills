...
- [ ] **gofmt**: Code is formatted with `gofmt` or `goimports` → [go-linting](go-linting.md)

---

## Documentation

- [ ] **Comment sentences**: Comments are full sentences starting with the name being described, ending with a period → [go-documentation](go-documentation.md)
- [ ] **Doc comments**: All exported names have doc comments; non-trivial unexported declarations too → [go-documentation](go-documentation.md)
- [ ] **Package comments**: Package comment appears adjacent to package clause with no blank line → [go-documentation](go-documentation.md)
- [ ] **Named result parameters**: Only used when they clarify meaning (e.g., multiple same-type returns), not just to enable naked returns → [go-documentation](go-documentation.md)
...
- [ ] **Interface location**: Define in consumer package, not implementor; return concrete types from producers → [go-interfaces](go-interfaces.md)
- [ ] **No premature interfaces**: Don't define before used; don't define "for mocking" on implementor side → [go-interfaces](go-interfaces.md)
- [ ] **Receiver type**: Use pointer if mutating, has sync fields, or is large; value for small immutable types; don't mix → [go-interfaces](go-interfaces.md)
...
- [ ] **Line length**: No rigid limit, but avoid uncomfortably long lines; break by semantics, not arbitrary length → [go-style-core](go-style-core.md)
- [ ] **Naked returns**: Only in short functions; explicit returns in medium/large functions → [go-style-core](go-style-core.md)
- [ ] **Pass values**: Don't use pointers just to save bytes; pass `string` not `*string` for small fixed-size types → [go-performance](go-performance.md)

---

## Imports

- [ ] **Import groups**: Standard library first, then blank line, then external packages → [go-packages](go-packages.md)
- [ ] **Import renaming**: Avoid unless collision; rename local/project-specific import on collision → [go-packages](go-packages.md)
- [ ] **Import blank**: `import _ "pkg"` only in main package or tests → [go-packages](go-packages.md)
- [ ] **Import dot**: Only for circular dependency workarounds in tests → [go-packages](go-packages.md)

---

## Testing

- [ ] **Examples**: Include runnable `Example` functions or tests demonstrating usage → [go-documentation](go-documentation.md)
- [ ] **Useful test failures**: Messages include what was wrong, inputs, got, and want; order is `got != want` → [go-testing](go-testing.md)
