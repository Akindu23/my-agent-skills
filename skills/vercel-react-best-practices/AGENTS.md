...
Comprehensive performance optimization guide for React and Next.js applications, designed for AI agents and LLMs. Documents **70 individual rules** across **8 categories** (see `rules/*.md`), prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule explains trade-offs and patterns to guide automated refactoring and code generation.
