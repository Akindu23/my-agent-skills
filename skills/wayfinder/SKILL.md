---
name: wayfinder
description: Wayfinder. Use when a chunk of work is too big for one agent session and the open decisions are still in fog, or to work through an existing decision map one ticket at a time.
---
...
**ADRs are promotion, not deliverables.** Resolving a ticket records the answer on the ticket (`## Answer` + Decisions-so-far gist). When that answer meets ADR-POLICY criteria, **offer** an ADR so durable project law lives in `docs/adr/` - the map stays the effort index, not a second copy of the ruling. Never delete or rewrite away a ticket because an ADR was written; tickets remain the debate trail.
...
The map is `work/<feature-slug>/map.md` - the canonical artifact. Its tickets are child files under `work/<feature-slug>/issues/`. Commit them - they are in-flight product work, not disposable scratch. If `.scratch/` still has tracker files, migrate into `work/` (same layout) before writing a new map.
...
The whole map at low resolution, loaded once per session. Live tickets are **not** listed in the map body - scan `issues/` for the **frontier** (unclaimed and unblocked).
...
Each ticket is a child file `issues/NN-<slug>.md` (numbered from `01`); the number is its identity. Create from this fence. `Type:` is `research` / `prototype` / `grilling` / `task` ([Ticket Types](#ticket-types)). `Status:` is written later (`claimed` or `resolved`). Add `Blocked by: NN, NN` under `Type:` in the wiring pass when it has blockers.
...
1. **Name the destination.** Run a `/grilling` and `/domain-modeling` session to pin down what this map is finding its way to - the spec, decision, or change. The destination fixes the scope, so it's settled first. **Resolved terms** before step 2. ADR offers wait for ticket close (see step 6).
...
3. **Create the map** (`work/<feature-slug>/map.md`): Destination and Notes filled in; Decisions-so-far empty for now; fog sketched into **Not yet specified**.
4. **Import locked decisions** when mid-session (or any charting that inherits prior accepts) - follow [Import locked decisions](references/import-locked-decisions.md); otherwise skip. Then **create the tickets you can specify now** from the [ticket fence](#tickets) as `issues/NN-<slug>.md` - then wire `Blocked by:` in a **second pass** (files need numbers before they can reference each other). Wiring sorts them into the frontier and the blocked; everything you can't yet specify stays in the fog - the **Not yet specified** section.
...
3. Resolve it - **zoom as needed**: fetch the full body of any related or resolved ticket on demand; invoke the skills the `## Notes` block names. If in doubt, use `/grilling` and `/domain-modeling`. While on this ticket, defer ADR offers from `/domain-modeling` to step 5. **Resolved terms** before step 4.
...
6. Add newly-surfaced tickets from the [ticket fence](#tickets) (create-then-wire); graduate any fog the answer has made specifiable, clearing each graduated patch from **Not yet specified** so it lives only as its new ticket. If the answer reveals a ticket - this one or another - sits beyond the destination, **rule it out of scope** rather than resolving it on the route. If the decision invalidates other parts of the map, update or delete those tickets.
