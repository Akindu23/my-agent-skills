...
2. On confirm: for each item, create a child from the [ticket fence](map-layout.md), append `## Answer`, set `Status: resolved`, and append it under **Decisions so far** (ticket link + one-line gist) - same shape as a normal resolve.
